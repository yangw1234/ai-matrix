...
def train(dataset, user_size, item_size):
    ...
    with tf.Session(config=config) as sess:
        ...
        model = NCF.NCF(FLAGS.embedding_size, user_size, item_size, FLAGS.lr,
                        ...
                        FLAGS.regularizer, dataset, FLAGS.topK, FLAGS.dropout, is_training=True)
        model.build()
        ...
        # print("Approximate accelerator performance in recommendations/second is: %.2f" % (
        # float(count * FLAGS.batch_size) / float(total_time)))
        ################################## SAVE MODEL ################################
...
def main(argv=None):
    ...
    test_data = NCF_input.eval_input_fn(test_features, test_labels,
                                        FLAGS.batch_size, user_negative, FLAGS.test_neg)

    dataset = TFDataset.from_tf_data_dataset(train_data, batch_size=FLAGS.batch_size, validation_dataset=test_data)

    # train_iterator = train_data.make_initializable_iterator()
    # test_iterator = test_data.make_initializable_iterator()

    if FLAGS.train_flag:
        train(dataset, user_size, item_size)
    else:
        infer(train_data, test_data, user_size, item_size)
